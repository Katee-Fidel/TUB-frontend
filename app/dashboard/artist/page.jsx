        ...
        <div className="max-w-5xl mx-auto px-8">
          ...
          <div className="flex items-center justify-between mt-10 mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/dashboard/artist/events/new"><button className="bg-marigold text-marigold-dark rounded-full px-5 py-2.5 text-sm font-extrabold hover:opacity-90 transition">+ Create event</button></Link>
            </div>
          </div>
          {loading && <p className="text-muted">Loading your events...</p>}
          {error && <p className="text-hibiscus">{error}</p>}
          {!loading && events.length === 0 && <div className="bg-surface border border-white/10 rounded-card p-10 text-center"><p className="text-muted">You haven&apos;t created any events yet.</p></div>}
