// All browser API calls intentionally use the same frontend origin.
// next.config.mjs proxies /api/* to BACKEND_URL, avoiding unreliable
// third-party cookies on mobile browsers.
const apiBase = "";
...
async function requestMultipart(path, method, formData, isRetry = false) {
  const res = await fetch(`${apiBase}${path}`, { method, credentials: "include", body: formData });
...
function buildEventFormData(fields, bannerFile) {
  ...
  Object.entries(fields).forEach(([key, value]) => { if (value !== undefined && value !== null && value !== "") formData.append(key, value); });
...
export const api = {
  ...
  getArtistDashboard: () => request("/api/artist-dashboard", { method: "GET" }),
  ...
  getWalletTransactions: () => request("/api/wallet/transactions", { method: "GET" }),
  ...
  updatePost: (id, text) => request(`/api/posts/${id}`, { method: "PATCH", body: JSON.stringify({ text }) }),
