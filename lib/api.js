...
export const api = {
  ...
  topupWallet: (amount, phone) =>
    request("/api/wallet/topup", {method: "POST", body: JSON.stringify({amount, phone})}),
  getTopupStatus: (checkoutRequestID) =>
    request(`/api/wallet/topup/${checkoutRequestID}/status`, {method: "GET"}),

  purchaseTicket: (eventId, payload) =>
    request(`/api/events/${eventId}/purchase`, {method: "POST", body: JSON.stringify(payload)}),
  ...
  validateTicket: (token) =>
    request("/api/tickets/validate", { method: "POST", body: JSON.stringify({ token }) }),
  ...
  createPost: (caption, image) => {
    ...
    return requestMultipart("/api/posts", "POST", formData);
  },
